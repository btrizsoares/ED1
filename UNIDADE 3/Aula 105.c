...
void inverter_pilha(Pilha *p) {
    if (vazia(p)) {
        printf("\n[STATUS] A pilha de comandos esta vazia. Nao ha feiticos para inverter.\n");
        return;
    }
    ...
    while (!vazia(p)) {
        Feitico extraido = pop(p);
        empilhar(&auxiliar, extraido.nome, extraido.custo_mana);
    }

    p->topo = auxiliar.topo;
    
    printf("\n[MAGIA TEMPORAL] Ordem dos feiticos invertida com sucesso!\n");
}
...
int main() {
    ...
    do {
        ...
        printf("6: Inverter Ordem dos Feiticos (Magia Temporal)\n"); 
        printf("7: Finalizar sequencia e sair\n");
